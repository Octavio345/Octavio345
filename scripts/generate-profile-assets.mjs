...
function agrotechProject() {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1400" height="660" viewBox="0 0 1400 660">
  <defs>
    <linearGradient id="b" x1="0" y1="0" x2="1400" y2="660"><stop stop-color="#050907"/><stop offset="1" stop-color="#0C2115"/></linearGradient>
    ...
    <style>.pipe{stroke-dasharray:8 12;animation:p 8s linear infinite}.scan{animation:s 4.5s ease-in-out infinite}.blink{animation:b 2.6s ease-in-out infinite}.float-inner{animation:f 4s ease-in-out infinite}@keyframes p{to{stroke-dashoffset:-240}}@keyframes s{0%,100%{transform:translateX(0);opacity:.15}50%{transform:translateX(180px);opacity:.8}}@keyframes b{0%,100%{opacity:.35}50%{opacity:1}}@keyframes f{0%,100%{transform:translateY(0)}50%{transform:translateY(-7px)}}</style>
  </defs>
  <rect width="1400" height="660" rx="34" fill="url(#b)"/><rect width="1400" height="660" rx="34" fill="url(#g)"/>
  <text x="70" y="76" fill="#73A982" font-family="Segoe UI,Arial,sans-serif" font-size="15" font-weight="800" letter-spacing="3">PROJETO EM DESTAQUE • TCC AGROTECH</text>
  <text x="70" y="128" fill="#F3FFF6" font-family="Segoe UI,Arial,sans-serif" font-size="46" font-weight="900">Zenith</text>
  <text x="70" y="168" fill="#B7FFC3" font-family="Segoe UI,Arial,sans-serif" font-size="24" font-weight="700">Sua precisão agrícola no ponto mais alto</text>
  <text x="70" y="206" fill="#9AB7A2" font-family="Segoe UI,Arial,sans-serif" font-size="17">Da captura da imagem ao resultado estruturado no backend.</text>

  <path class="pipe" d="M258 370H418M626 370H786M994 370H1150" fill="none" stroke="#63E67A" stroke-width="2" opacity=".52"/>
  <g transform="translate(70 260)"><g class="float-inner"><rect width="188" height="220" rx="24" fill="#08150D" stroke="#2A5D3B"/>
    ...
    <text x="24" y="172" fill="#F3FFF6" font-family="Segoe UI,Arial,sans-serif" font-size="18" font-weight="800">Campo / Drone</text><text x="24" y="197" fill="#92AE99" font-family="Segoe UI,Arial,sans-serif" font-size="13">imagens da lavoura</text></g></g>

  <g transform="translate(418 260)"><rect width="208" height="220" rx="24" fill="#08150D" stroke="#2A5D3B"/><text x="24" y="34" fill="#73A982" font-family="Segoe UI,Arial,sans-serif" font-size="12" font-weight="800" letter-spacing="2">02 • PROCESSAMENTO</text>
  ...
  <g transform="translate(786 260)"><rect width="208" height="220" rx="24" fill="#08150D" stroke="#2A5D3B"/><text x="24" y="34" fill="#73A982" font-family="Segoe UI,Arial,sans-serif" font-size="12" font-weight="800" letter-spacing="2">03 • INTELIGÊNCIA</text>
  ...
  <g transform="translate(1150 260)"><rect width="180" height="220" rx="24" fill="#08150D" stroke="#2A5D3B"/><text x="24" y="34" fill="#73A982" font-family="Segoe UI,Arial,sans-serif" font-size="12" font-weight="800" letter-spacing="2">04 • API</text>
...
function stackSvg() {
  ...
  const cards = profileStack.map((item, index) => {
    const row = Math.floor(index / columns);
    const col = index % columns;
    const rowCount = Math.min(columns, profileStack.length - row * columns);
    const rowWidth = rowCount * cardWidth + (rowCount - 1) * gapX;
    const rowStartX = (1400 - rowWidth) / 2;
    const x = rowStartX + col * (cardWidth + gapX);
    const y = startY + row * (cardHeight + gapY);
    const delay = (index * 0.09).toFixed(2);
    const fontSize = item.label.length > 14 ? 13 : 15;
    return `<g transform="translate(${x} ${y})">
      <rect width="${cardWidth}" height="${cardHeight}" rx="22" fill="#07110C" stroke="#244D33"/>
      <rect x="1" y="1" width="${cardWidth - 2}" height="5" rx="3" fill="${item.accent}" opacity=".92"/>
      <rect x="45" y="19" width="80" height="61" rx="18" fill="#0B1710" stroke="#284D35"/>
      <g class="logo-pulse" style="animation-delay:${delay}s">${item.icon}</g>
      <text x="85" y="105" text-anchor="middle" fill="#F1FFF5" font-family="Segoe UI,Arial,sans-serif" font-size="${fontSize}" font-weight="800">${esc(item.label)}</text>
    </g>`;
  }).join("");

  return `<svg xmlns="http://www.w3.org/2000/svg" width="1400" height="650" viewBox="0 0 1400 650">
  <defs>
    <linearGradient id="stackBg" x1="0" y1="0" x2="1400" y2="650">
      <stop stop-color="#050907"/><stop offset=".55" stop-color="#07130C"/><stop offset="1" stop-color="#0A1D12"/>
    </linearGradient>
    <radialGradient id="stackGlow" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(1110 76) rotate(150) scale(420 180)">
      <stop stop-color="#63E67A" stop-opacity=".16"/><stop offset="1" stop-color="#63E67A" stop-opacity="0"/>
    </radialGradient>
    <pattern id="stackGrid" width="42" height="42" patternUnits="userSpaceOnUse">
      <path d="M42 0H0V42" fill="none" stroke="#193422" stroke-width="1" opacity=".32"/>
    </pattern>
    <style>
      .data-line{stroke-dasharray:8 15;animation:dash 18s linear infinite}
      .logo-pulse{animation:pulse 4.8s ease-in-out infinite;transform-origin:center;transform-box:fill-box}
      @keyframes dash{to{stroke-dashoffset:-360}}
      @keyframes pulse{0%,100%{opacity:.84}50%{opacity:1}}
    </style>
  </defs>
  <rect width="1400" height="650" rx="32" fill="url(#stackBg)"/>
  <rect width="1400" height="650" rx="32" fill="url(#stackGrid)"/>
  <rect width="1400" height="650" rx="32" fill="url(#stackGlow)"/>

  <text x="78" y="67" fill="#F3FFF6" font-family="Segoe UI,Arial,sans-serif" font-size="32" font-weight="900">Tecnologia de ponta a ponta</text>
  <text x="78" y="98" fill="#92AE99" font-family="Segoe UI,Arial,sans-serif" font-size="15">Tecnologias que uso em projetos web, backend, banco de dados, IA e visão computacional.</text>
  <path d="M78 124H1322" stroke="#244D33"/>
  <path class="data-line" d="M1010 124H1322" stroke="#63E67A" stroke-width="2" opacity=".42"/>
  <circle cx="1168" cy="124" r="4" fill="#7CFF95"/><circle cx="1198" cy="124" r="4" fill="#63E67A" opacity=".7"/><circle cx="1228" cy="124" r="4" fill="#63E67A" opacity=".35"/>
  ${cards}
  <text x="700" y="624" text-anchor="middle" fill="#63846C" font-family="Segoe UI,Arial,sans-serif" font-size="12" letter-spacing="1.2">WEB  •  BACKEND  •  BANCO DE DADOS  •  IA  •  VISÃO COMPUTACIONAL  •  VERSIONAMENTO</text>
</svg>`;
}
